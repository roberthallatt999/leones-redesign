...
use Low\Reorder\FluxCapacitor\Base\Mcp;
 ...
 */
class Low_reorder_mcp extends Mcp
{
    // Use the base trait
    use Low_reorder_base;
    ...
    public function settings()
    {
        ...
        if (version_compare(APP_VER, '6.0', '>=')) {
            ...
            $groups = ee('Model')
                ...
                ->all()->getDictionary('group_id', 'group_title');
        }
    ...
    public function edit($set_id = 'new')
    {
        ...
        if (ee4()) {
            $fields = ee('Model')
                ->get('ChannelField')
                ->filter('site_id', 'IN', array($this->site_id, 0))
                ->order('field_label')
                ->all();
    ...
    */
    public function save_set()
    {
        ...
        if ($search = ee('Request')->post('lrsearch')) {
            foreach ($search['fields'] as $i => $key) {
                // Skip invalid
                if (empty($search['values'][$i])) {
                    continue;
                }
    ...
    */
    public function reorder($set_id, $cat_id = 0)
    {
        ...
        if ($set['cat_option'] == 'one' && ! empty($set['cat_groups'])) {
            ...
            // URL to jump to
            $this->data['url'] = $this->mcp_url("reorder/{$set_id}/%s");
        ...
        if ($this->data['show_entries']) {
            ...
            // Edit entry url
            $edit_tmpl = '<a href="' . ee('CP/URL', 'publish/edit/entry/%d')->compile() . '">%s</a>';
    ...
    public function save_order()
    {
        ...
        $entries = explode(',', ee('Request')->post('sort_order'));
    ...
    {
        $this->data['ee_ver'] = substr(APP_VER, 0, 1);
        $this->data['action_url'] = $this->flux->moduleURL('save_license');
        $this->data['license_key'] = $this->license_key;
        $this->data['ignore_site'] = $this->ignore_site;
