#!/bin/bash
#
# Phase 2 — Copy existing images/ into shared/images/ on leones-dev.
#
# Run this AFTER the first GitHub Actions deploy has succeeded (so a
# releases/<ts>/ directory and a `current` symlink exist) and BEFORE
# you change the Forge nginx webroot to ${REMOTE_BASE_PATH}/current.
#
...
if [ "${RELEASES_COUNT}" -eq 0 ]; then
    ...
    yellow "Run the first GitHub Actions deploy before continuing."
    exit 1
fi
...
# ----------------------------------------------------------------------------
# Step 4: verify the symlink in current/ points to a populated shared/images
# ----------------------------------------------------------------------------
...
Next step:
  ...
  Forge UI: Sites → leones-dev → Meta → Web Directory
  After saving, Forge reloads nginx automatically.

  Then load the site in a browser, click through 3-4 pages, confirm:
    - CSS loads
